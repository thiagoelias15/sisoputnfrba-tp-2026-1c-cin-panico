...
./matar.sh
sleep 1
echo "=== Iniciando Prueba Memoria (WORST FIT) ==="

xterm -T "SWAP" -e bash -c "$BASE/swap/bin/swap $BASE/swap/swap.config; echo 'SWAP terminó'; read" &
sleep 1
xterm -T "KERNEL MEMORY" -e bash -c "$BASE/kernel_memory/bin/kernel_memory $BASE/kernel_memory/memoria_memoria_worst.config; echo 'KM terminó'; read" &
sleep 1
...
echo "=== Prueba Memoria WORST iniciada (4 sticks: 16+32+64+128) ==="